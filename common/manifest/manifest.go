 ...
 *
 */
func (m *Manifest) AddNode(ip string, port string, pubKey string, chainAddress map[string]string) error {
	...
	if err != nil {
		m.Nodes = append(m.Nodes, &TrustNode{ip, port, pubKey,  "ADDED", chainAddress})
		m.N++
		return nil
	}
