...
func (c *EthereumCoin) Attach() error {
	...
	ethereumClient, err := ethclient.Dial("http://" + viper.GetString("ETHEREUM_RPC"))
	if err != nil {
		return err
	}
...
func (c *EthereumCoin) GetDepositsInBlock(blockID int64, trustAddress string) ([]*Deposit, error) {
	ndeposits, err := c.client.GetNativeDeposits(blockID, "")
	if err != nil {
		return nil, err
	}
	deps, err := c.client.FilterTransferEvent(blockID, "")
	if err != nil {
		return nil, err
	}
