...
// GetConfig retrieves compile-time constants
func (api *API) GetKeyReferences(pubkey string) ([][]types.ObjectID, error) {
	var config [][]types.ObjectID
	err := api.call("get_key_references", []interface{}{[]interface{}{pubkey}}, &config)
	return config, err
}
